...
def restore(dirc):
	...
	if(r1[0] == 'RSR'):
		...
		print 'BS: '+response
		splitresponse = response.split(' ')
		if (splitresponse[0] == 'AUR' and splitresponse[1] == 'OK\n'):
			print 'entrou no if'
			b.send('RSB '+ dirc + '\n'.encode())
			print 'ola'
			c = b.recv(1)
			...
			#NO DECODE
				...
				if (not readingfile):
					if (c == ' '):
						...
						if(cont == 1):
							if (response == 'RBR'):
								print 'esta a entrar no status check'
								status = 1
								print response
								response = ''
						elif (cont == 2):
							numberoffiles = int(response)
							print numberoffiles
							response = ''
						elif (cont == numberoffilesdone * 5 + 1 + 2):
							filename = response
							response = ''
						elif (cont == numberoffilesdone * 5 + 2 + 2):
							filedate = response
							response = ''
						elif (cont == numberoffilesdone * 5 + 3 + 2):
							filetime = response
							response = ''
						elif (cont == numberoffilesdone * 5 + 4 + 2):
				...
				if (readingfile):
					...
					if not os.path.exists('./'+dirc):
						os.makedirs('./'+dirc)
					try:
						os.remove(dirc+'/'+filename)
						print 'File '+filename+' overwritten'
					except:
						print 'file' + filename +' created'
					file = open(dirc+'/'+filename, 'wb+')
					file.write(filedata)
					file.close()
					filedata =''
					numberoffilesdone += 1
...
while True:	
	...
	if num_of_spaces == 1:
		...
		#--------------RESTORE---------------#
		if(processed_input[0]== 'restore'):
			print processed_input[1]
			if(aurState == 'AUR OK\n'):
				print aurState
				restore(processed_input[1])
			if(aurState == 'AUR NEW\n'):
				print aurState
				login(user,password)
				restore(processed_input[1])
